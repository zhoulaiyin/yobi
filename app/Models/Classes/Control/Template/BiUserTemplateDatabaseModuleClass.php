...
use App\Models\Control\Template\BiUserTemplateDatabaseModule;

class BiUserTemplateDatabaseModuleClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = BiUserTemplateDatabaseModule::where($where)
            ->count();

        $results = BiUserTemplateDatabaseModule::where($where)
    ...
    {
        $BiUserTemplateDatabaseModule = BiUserTemplateDatabaseModule::find($id);
        if (empty($BiUserTemplateDatabaseModule)) {
            return null;
        }

        return $BiUserTemplateDatabaseModule->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = BiUserTemplateDatabaseModule::find($id);
        } else {
            //新增
            $result = new BiUserTemplateDatabaseModule();
    ...
    public static function del($_id)
    {
        ...
        $res = BiUserTemplateDatabaseModule::find($_id);
        if (!$res) {
