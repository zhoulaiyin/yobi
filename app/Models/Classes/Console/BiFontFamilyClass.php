...
use App\Models\Console\BiFontFamily;


class BiFontFamilyClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = BiFontFamily::where($where)
            ->count();

        $results = BiFontFamily::where($where)
    ...
    {
        $BiFontFamily = BiFontFamily::find($id);
        if (empty($BiFontFamily)) {
            return null;
        }

        return $BiFontFamily->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = BiFontFamily::find($id);
        } else {
            //新增
            $result = new BiFontFamily();
