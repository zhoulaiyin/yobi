...
use App\Models\Console\BiTemplateDatabase;

class BiTemplateDatabaseClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = BiTemplateDatabase::where($where)
            ->count();

        $results = BiTemplateDatabase::where($where)
    ...
    {
        $BiTemplateDatabase = BiTemplateDatabase::find($id);
        if (empty($BiTemplateDatabase)) {
            return null;
        }

        return $BiTemplateDatabase->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = BiTemplateDatabase::find($id);
        } else {
            //新增
            $result = new BiTemplateDatabase();
    ...
    public static function del($_id)
    {
        ...
        $res = BiTemplateDatabase::find($_id);
        if (!$res) {
