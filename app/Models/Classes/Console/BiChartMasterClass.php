...
use App\Models\Console\BiChartMaster;

class BiChartMasterClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = BiChartMaster::where($where)
            ->count();

        $results = BiChartMaster::where($where)
    ...
    {
        $BiChartMaster = BiChartMaster::find($id);
        if (empty($BiChartMaster)) {
            return null;
        }

        return $BiChartMaster->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = BiChartMaster::find($id);
        } else {
            //新增
            $result = new BiChartMaster();
