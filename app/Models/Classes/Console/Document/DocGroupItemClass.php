...
use App\Models\Console\Document\DocGroupItem;

class DocGroupItemClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = DocGroupItem::where($where)
            ->count();

        $results = DocGroupItem::where($where)
    ...
    {
        $DocGroupItem = DocGroupItem::find($id);
        if (empty($DocGroupItem)) {
            return null;
        }

        return $DocGroupItem->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = DocGroupItem::find($id);
        } else {
            //新增
            $result = new DocGroupItem();
