...
use App\Models\Console\System\SysTask;
...
class SysTaskClass
{
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = SysTask::where($where)
            ->count();

        $results = SysTask::where($where)
    ...
    {
        $SysTask = SysTask::find($id);
        if (empty($SysTask)) {
            return null;
        }

        return $SysTask->toArray();
    }
