...
use App\Models\Console\System\SysTaskLog;

class SysTaskLogClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = SysTaskLog::where($where)
            ->count();

        $results = SysTaskLog::where($where)
    ...
    {
        $SysTaskLog = SysTaskLog::find($id);
        if (empty($SysTaskLog)) {
            return null;
        }

        return $SysTaskLog->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = SysTask::find($id);
        } else {
            //新增
            $result = new SysTask();
            $result->creator = $WS->UserId();
        }
