...
use App\Models\Console\BiTemplateGroup;

class BiTemplateGroupClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = BiTemplateGroup::where($where)
            ->count();

        $results = BiTemplateGroup::where($where)
    ...
    {
        $BiTemplateGroup = BiTemplateGroup::find($id);
        if (empty($BiTemplateGroup)) {
            return null;
        }

        return $BiTemplateGroup->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = BiTemplateGroup::find($id);
        } else {
            //新增
            $result = new BiTemplateGroup();
