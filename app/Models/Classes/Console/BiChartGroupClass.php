...
use App\Models\Console\BiChartGroup;

class BiChartGroupClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = BiChartGroup::where($where)
            ->count();

        $results = BiChartGroup::where($where)
    ...
    {
        $BiChartGroup = BiChartGroup::find($id);
        if (empty($BiChartGroup)) {
            return null;
        }

        return $BiChartGroup->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = BiChartGroup::find($id);
        } else {
            //新增
            $result = new BiChartGroup();
