...
use App\Models\Console\BiRuleGroup;

class BiRuleGroupClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = BiRuleGroup::where($where)
            ->count();

        $results = BiRuleGroup::where($where)
    ...
    {
        $BiRuleGroup = BiRuleGroup::find($id);
        if (empty($BiRuleGroup)) {
            return null;
        }

        return $BiRuleGroup->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = BiRuleGroup::find($id);
        } else {
            //新增
            $result = new BiRuleGroup();
