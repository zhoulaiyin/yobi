...
class BiRuleClass
{
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = BiRule::where($where)
            ->count();

        $results = BiRule::where($where)
    ...
    {
        $BiRule = BiRule::find($id);
        if (empty($BiRule)) {
            return null;
        }

        return $BiRule->toArray();
    }
    ...
    {
        $BiRule = BiRule::where($where)->get()->first();
        if (empty($BiRule)) {
            return null;
        }
