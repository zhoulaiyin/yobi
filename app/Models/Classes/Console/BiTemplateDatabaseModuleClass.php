...
namespace App\Models\Classes\Console;

use App\Models\Console\BiTemplateDatabaseModule;

class BiTemplateDatabaseModuleClass
{
    /**
     * 查询数据
    ...
    public static function getList($where = [], $limit = [], $columns = '*')
    {
        ...
        $total_num = BiTemplateDatabaseModule::where($where)
            ->count();

        $results = BiTemplateDatabaseModule::where($where)
    ...
    {
        $BiTemplateDatabaseModule = BiTemplateDatabaseModule::find($id);
        if (empty($BiTemplateDatabaseModule)) {
            return null;
        }

        return $BiTemplateDatabaseModule->toArray();
    }
    ...
    public static function save($args, $id = null)
    {
        ...
        if (!empty($id)) {
            //编辑
            $result = BiTemplateDatabaseModule::find($id);
        } else {
            //新增
            $result = new BiTemplateDatabaseModule();
    ...
    public static function del($_id)
    {
        ...
        $res = BiTemplateDatabaseModule::find($_id);
        if (!$res) {
